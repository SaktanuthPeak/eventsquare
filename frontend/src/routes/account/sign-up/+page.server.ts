...
export const actions: Actions = {
	default: async (event) => {
		...
		if (!createUserRes.response.ok) {
			...
			switch (statusCode) {
				case 400:
					errorMessage = errorMessage || 'ข้อมูลไม่ถูกต้อง กรุณาตรวจสอบข้อมูลที่กรอก';
					break;
				case 401:
				...
				case 409:
					...
					break;
				case 422:
					errorMessage = errorMessage || 'ข้อมูลไม่ถูกต้องตามเงื่อนไข กรุณาตรวจสอบข้อมูลที่กรอก';
					break;
				default:
			...
			return fail(statusCode, {
				form,
				type: 'error',
				statusCode: statusCode, // Add this explicitly
